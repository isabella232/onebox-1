    ...
    module HTMLEmbed
      ...
      def og_raw
        ...
        if oembed_alternate
          ...
          begin
            @og_raw = Onebox::Helpers.symbolize_keys(::MultiJson.load(fetch_response(oembed_alternate['href']).body))
        ...
        if @og_raw
          @og_raw[:image] = open_graph.images.first if @og_raw[:image].nil? && open_graph && open_graph.images
